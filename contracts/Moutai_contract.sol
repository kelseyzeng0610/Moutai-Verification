...
contract Asset {
    ...
    event OwnershipTransferAccepted(uint256 indexed propertyId, address indexed newOwner);
    event OwnershipTransferInitiated(uint256 indexed propertyId, address indexed newOwner);
    ...
    mapping(uint256 => BottleDetail) public properties; // Stores all properties propId -> property Detail
    ...
    function createProperty(
     ...
     {
        properties[_propId] = BottleDetail(Status.Pending, _value, _owner);
        return true;
    }
    ...
    function initOwnershipTransfer(uint256 propertyId, address currentOwner, address newOwner) public {
        ...
        emit OwnershipTransferInitiated(propertyId,currentOwner);
    }
    function transferOwnership(uint256 propertyId, address payable currentOwner, address payable newOwner) public payable {
    require(properties[propertyId].currOwner == currentOwner, "Only the owner can initiate the ownership transfer.");
    require(newOwner != address(0), "New owner cannot be the zero address.");
    require(msg.sender == newOwner, "Only the new owner can call this function.");
    uint256 balance = msg.sender.balance;
    require(balance >= properties[propertyId].value, "Insufficient funds for the property value.");
    require(pendingOwner[propertyId] == newOwner);

    if(msg.value == properties[propertyId].value * 1 ether) {
        properties[propertyId].currOwner = newOwner;
        delete pendingOwner[propertyId];
        } else {
